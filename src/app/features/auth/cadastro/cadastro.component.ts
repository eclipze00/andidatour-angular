...
@Component({
  ...
  standalone: true,
  imports: [RouterLink, FormsModule, CommonModule, ToastComponent], // ← ToastComponent adicionado
  template: `
      ...
      <div class="flex items-center justify-center p-6 md:p-10" style="background:var(--background)">
        <div class="w-full max-w-sm space-y-6">
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-1.5">
              <label class="text-sm font-medium">E-mail profissional</label>
            ...
            <div class="space-y-1.5">
              ...
              <div class="relative">
                <input [(ngModel)]="password" [type]="showPass ? 'text' : 'password'" class="input-field pr-10" placeholder="Mínimo 8 caracteres" />
                <button type="button" (click)="showPass = !showPass" class="absolute right-3 top-1/2 -translate-y-1/2 text-sm" style="color:var(--muted-foreground)">
                  {{ showPass ? '🙈' : '👁' }}
                </button>
              </div>
              <div class="flex gap-1 mt-1.5" *ngIf="password">
                <div *ngFor="let s of [0,1,2,3]" class="h-1 flex-1 rounded-full transition-colors"
            ...
            <div class="space-y-1.5">
              ...
              <span *ngIf="submitted && confirmPassword && password !== confirmPassword" class="text-xs" style="color:var(--destructive)">As senhas não coincidem</span>
            </div>
            <label class="flex items-start gap-2.5 cursor-pointer">
              <input type="checkbox" [(ngModel)]="acceptTerms" class="mt-0.5 h-4 w-4 rounded" />
              <span class="text-xs" style="color:var(--muted-foreground)">
                Concordo com os <a href="#" class="hover:underline font-medium" style="color:var(--brand)">Termos de uso</a>
                e a <a href="#" class="hover:underline font-medium" style="color:var(--brand)">Política de privacidade</a>
              </span>
            </label>
            <span *ngIf="submitted && !acceptTerms" class="text-xs block -mt-2" style="color:var(--destructive)">Você precisa aceitar os termos</span>
          </div>
          ...
          <p class="text-center text-sm" style="color:var(--muted-foreground)">
            Já tem conta? <a routerLink="/login" class="font-medium hover:underline" style="color:var(--brand)">Entrar</a>
          </p>
        </div>
      </div>
    </div>
    <app-toast />  <!-- ← fora do grid, fora do layout -->
  `,
  styles: [`
    .input-field { width:100%; border-radius:var(--radius); border:1px solid var(--border); padding:0.5rem 0.75rem; font-size:0.875rem; outline:none; background:white; transition:border-color 0.15s; }
    .input-field:focus { border-color:var(--brand); box-shadow:0 0 0 2px oklch(0.48 0.15 255 / 0.15); }
  `]
})
export class CadastroComponent {
  ...
  isValid(): boolean {
    return !!(this.firstName && this.lastName && this.email && this.password.length >= 8 && this.password === this.confirmPassword && this.acceptTerms);
  }
  ...
  onSubmit() {
    ...
    this.loading = true;
    this.cdr.detectChanges();  // ← força botão loading
    this.auth.register({ firstName: this.firstName, lastName: this.lastName, email: this.email, password: this.password }).subscribe({
      ...
      error: () => {
        ...
        this.cdr.detectChanges();  // ← destrava botão e mostra toast
      }
    });
  }
}